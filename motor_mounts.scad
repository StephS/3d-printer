...
union() {
	...
	difference() {
		union() {
			...
			difference() {
				translate([(z_screw_rod_separation+stepper_motor_padded/2), -support_wall_thickness-stepper_motor_padded/2, motor_mount_thickness/2]) cylinder(r=z_smooth_rod_diameter/2+4,h=motor_mount_thickness, center = true);
		...
		// smooth rod hole
		translate([(z_screw_rod_separation+stepper_motor_padded/2), -support_wall_thickness-stepper_motor_padded/2, motor_mount_thickness/2]) rod_hole(d=z_smooth_rod_diameter, h=motor_mount_thickness+1, allowance=0.05, center = true);
