...
translate([0,0, extrusion[0]*2]) {
	...
	translate([0, y_length/2-z_extrusion_pos, 0]) {
		...
		translate([0, -support_wall_thickness-stepper_motor_padded/2-extrusion[0]/2, z_height/2]) {
			translate([(x_width/2 +(z_screw_rod_separation+stepper_motor_padded/2)), 0, 52 + 2 * bushing_x[0]-3]) rotate([180, 0, -90]) x_end_idler(thru=true);
			translate([-(x_width/2 +(z_screw_rod_separation+stepper_motor_padded/2)),0 ,52 + 2 * bushing_x[0]-3]) rotate([180, 0, -90]) x_end_motor();
	
			// x axis smooth rod
			// the width of the rod block is 50, we have 2, so add in 100
			translate([0, 10 + bushing_x[0], 0]) {
				translate([+4, 0, 6]) rotate([0, -90, 0]) color("DimGray") cylinder(h = x_smooth_rod_length, r=x_smooth_rod_diameter/2, center=true);
				translate([+4, 0, xaxis_rod_distance+6]) rotate([0, -90, 0]) color("DimGray") cylinder(h = x_smooth_rod_length, r=x_smooth_rod_diameter/2, center=true);
				//X axis carriage
	...
	translate([(pulley[5]+y_idler_bearing[0])/4, 0, y_bushing_height+y_rod_height+y_smooth_rod_diameter/2]) rotate(a=[0,180,-90]) i3_belt_clamp();
