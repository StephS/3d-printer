...
module x_end_base(vfillet=[3, 3, 3, 3], thru=true, len=40, offset=0){
    ...
    difference(){
        ...
        // belt hole
        translate([-5.5 - 10 + 1.5, 22 - 9 + offset, 30]) cube([max(x_idler_width + 2, 11), 55, 27], center = true);

        translate([-10 - bushing_xy[0], offset, 0]) {
            if(thru == true){
                translate([0, -11, 6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.1, 50);
                translate([0, -11, xaxis_rod_distance+6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.1, 50);
            } else {
                translate([0, -7, 6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.1, 50);
                translate([0, -7, xaxis_rod_distance+6]) rotate([-90, 0, 0]) pushfit_rod(bushing_xy[0] * 2 + 0.1, 50);
            }
        }
