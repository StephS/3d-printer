...
module y_rod_mount(height=y_rod_height) {
	difference() {
		...
		translate([0, -support_wall_thickness-height, 0]) rod_hole(d=y_smooth_rod_diameter, h=extrusion[0]+1, allowance=0.05, center = true);
...
translate([0, 0, extrusion[0]/2]) {
	...
	translate([y_smooth_rod_diameter+support_wall_thickness*2+screw_head_top_dia(v_screw_hole(ex_screw))+4, y_rod_height+support_wall_thickness*2+y_smooth_rod_diameter/2+2, 0]) rotate(a=[0,0,0]) y_rod_mount();

	translate([0, y_rod_height+support_wall_thickness*2+y_smooth_rod_diameter/2+2, 0]) {
		translate([0, 2, 0]) rotate(a=[0,0,180]) y_rod_mount();
		translate([y_smooth_rod_diameter+support_wall_thickness*2+screw_head_top_dia(v_screw_hole(ex_screw))+4, y_rod_height+support_wall_thickness*2+y_smooth_rod_diameter/2+2, 0]) rotate(a=[0,0,0]) y_rod_mount();
	}
