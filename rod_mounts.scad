...
translate([-(ex_screw_head_dia_padded+y_idler_bearing[0]+extrusion[0])/2 -smooth_rod_diameter/2-support_wall_thickness -4, extrusion[0]/2 + 10 +support_wall_thickness*2, 0]) rotate(a=[0,0,90]) y_idler_mount();

// Y rod mount (print 4)
translate([0, 2, extrusion[0]/2]) rotate(a=[0,0,180]) y_rod_mount();
translate([((smooth_rod_diameter+support_wall_thickness*2+ex_screw_head_dia_padded) > (end_x  + smooth_rod_diameter + support_wall_thickness*2+2) ? (smooth_rod_diameter+support_wall_thickness*2+ex_screw_head_dia_padded) : (end_x  + smooth_rod_diameter + support_wall_thickness*2+2)) + 4, 6+support_wall_thickness*2, extrusion[0]/2]) y_rod_mount();
translate([-(((smooth_rod_diameter+support_wall_thickness*2+ex_screw_head_dia_padded) > (end_x  + smooth_rod_diameter + support_wall_thickness*2+2) ? (smooth_rod_diameter+support_wall_thickness*2+ex_screw_head_dia_padded) : (end_x  + smooth_rod_diameter + support_wall_thickness*2+2)) + 4), 6+support_wall_thickness*2, extrusion[0]/2]) y_rod_mount();
translate([smooth_rod_diameter+support_wall_thickness*2+ex_screw_head_dia_padded+4, 10+support_wall_thickness*2, extrusion[0]/2]) rotate(a=[0,0,180]) y_rod_mount();
